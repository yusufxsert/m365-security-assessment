...
.NOTES
    Required Graph Permissions : Application.Read.All
...
function Test-AppRegistrations {
    ...
    try {
        $uri = 'https://graph.microsoft.com/v1.0/applications?$select=id,appId,displayName,signInAudience,passwordCredentials,keyCredentials,requiredResourceAccess&$top=999'
        do {
            $response = Invoke-MgGraphRequest -Method GET -Uri $uri -ErrorAction Stop
            foreach ($app in $response.value) { $apps.Add($app) }
    ...
    # APP-002: Long-lived credentials (> 1 year validity)
    # -------------------------------------------------------------------------
    $veryLongLived = [System.Collections.Generic.List[string]]::new()   # > 2 years
    $longLived     = [System.Collections.Generic.List[string]]::new()   # > 1 year
    ...
    if ($multiTenantApps.Count -gt 0) {
        # Flag those with broad API permissions as higher risk
        # Dangerous permission GUIDs (well-known, static across tenants):
        $dangerousRoles = @(
            '1bfefb4e-e0b5-418b-a88f-73c46d2cc8e9',  # Application.ReadWrite.All
            '19dbc75e-c2e2-444c-a770-ec69d8559fc7',  # Directory.ReadWrite.All
            'e2a3a72e-5f79-4c64-b1b1-878b674786c9',  # Mail.ReadWrite (app)
            'b633e1c5-b582-4048-a93e-9f11b44c7e96',  # Mail.Send (app)
    ...
    # -------------------------------------------------------------------------
    # APP-004: Apps with Global Admin / privileged API permissions (application type)
    # -------------------------------------------------------------------------
    # Well-known dangerous application permission GUIDs
    $privilegedRoleMap = @{
        '1bfefb4e-e0b5-418b-a88f-73c46d2cc8e9' = 'Application.ReadWrite.All'
        '19dbc75e-c2e2-444c-a770-ec69d8559fc7' = 'Directory.ReadWrite.All'
    ...
    # -------------------------------------------------------------------------
    # APP-005: App registrations with no owners (that have admin-consent permissions)
    # -------------------------------------------------------------------------
    # Only check apps that have requiredResourceAccess entries (i.e., they request permissions)
    ...
    foreach ($app in $appsWithPerms) {
        try {
            $ownerResponse = Invoke-MgGraphRequest -Method GET `
                -Uri "https://graph.microsoft.com/v1.0/applications/$($app.id)/owners?`$select=id,displayName&`$top=5" `
                -ErrorAction Stop
            if ($ownerResponse.value.Count -eq 0) {
    ...
    # -------------------------------------------------------------------------
    # Retrieve all service principal appIds for cross-reference
    $spAppIds = [System.Collections.Generic.HashSet[string]]::new()
    try {
        $spUri = 'https://graph.microsoft.com/v1.0/servicePrincipals?$select=appId&$top=999'
        do {
            $spResponse = Invoke-MgGraphRequest -Method GET -Uri $spUri -ErrorAction Stop
            foreach ($sp in $spResponse.value) { [void]$spAppIds.Add($sp.appId) }
