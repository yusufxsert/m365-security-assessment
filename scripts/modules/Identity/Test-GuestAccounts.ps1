...
<#
.SYNOPSIS
    Checks Entra ID guest account hygiene and external access risk.
.DESCRIPTION
    ...
    Note: GST-003 (invite settings) and GST-004 (guest role level) overlap with
    the existing Checks/Entra/Check-GuestAccessSettings.ps1 but provide
    CheckId-tagged results for the structured assessment framework.
.NOTES
...
function Test-GuestAccounts {
    ...
    # Fetch guest and member counts once for GST-001
    $allGuests = $null
    $totalUserCount = 0
    ...
    try {
        ...
        # Total users (members + guests) for percentage calculation
    ...
    try {
        ...
        $staleGuests = $guests | Where-Object {
            ...
            # Never signed in OR signed in more than 90 days ago
    ...
    try {
        ...
        # allowInvitesFrom values: admins | adminsAndGuestInviters | adminsGuestInvitersAndMembers | everyone
    ...
    try {
        ...
        # Known GUIDs for guest access levels
        # 10dae51f-b6af-4016-8d66-8c2a99b929b3 = Restricted Guest User (most restrictive)
        # bf6b4499-ff22-4af0-a0ee-2c280a9d66bb = Guest User (default, limited access)
        # a0b1b346-4d3e-4e8b-98f8-753987be4970 = Member-like access (same as member - least restrictive)
