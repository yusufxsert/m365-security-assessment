...
<#
.SYNOPSIS
    Checks Entra ID user account hygiene and identity risk indicators.
.DESCRIPTION
...
.NOTES
    Required Permissions:
        ...
        Directory.Read.All
    License: E3 minimum; AuditLog requires E3+
    See also (PS-only variant — no App Registration required):
        scripts/modules-psonly/Identity/Test-UserIdentities.ps1
...
function Test-UserIdentities {
    ...
    try {
        ...
        if ($totalEnabled -eq 0) {
            ...
            $pct = [math]::Round(($unlicensedCount / $totalEnabled) * 100, 1)
        }
        ...
        $results.Add((New-AssessmentResult `
            ...
            -Recommendation 'Grant User.Read.All to the service principal.' `
            -Reference 'https://learn.microsoft.com/entra/identity/users/licensing-groups-resolve-problems' `
            -Category  'Identity' `
            -Severity  'Info'))
    }
    ...
    try {
        ...
        # signInActivity requires AuditLog.Read.All; not filterable server-side, so fetch all and filter locally
        $enabledUsers = Get-MgUser -Filter "accountEnabled eq true" `
        ...
        $staleUsers = $enabledUsers | Where-Object {
            ...
            # Accounts that have never signed in, or signed in before the cutoff
            $null -eq $lastSignIn -or $lastSignIn -lt (Get-Date).AddDays(-90)
        }

        # Check if any stale users are admins
        ...
        $results.Add((New-AssessmentResult `
            ...
            -Recommendation 'Grant User.Read.All and AuditLog.Read.All to the service principal.' `
            -Reference 'https://learn.microsoft.com/entra/identity/users/clean-up-stale-guest-accounts' `
            -Category  'Identity' `
            -Severity  'Info'))
    }
    ...
    try {
        ...
        $adminPct = if ($totalUsers -gt 0) {
            $adminTotal = ($regDetails | Where-Object { $_.IsAdmin -eq $true }).Count
            if ($adminTotal -gt 0) { [math]::Round(($adminsWithoutMfa.Count / $adminTotal) * 100, 1) } else { 0 }
        } else { 0 }
    ...
    try {
        # Query sign-ins with riskLevelAggregated of high or medium in last 30 days
        $thirtyDaysAgo = (Get-Date).AddDays(-30).ToString('yyyy-MM-ddTHH:mm:ssZ')
        $riskySignIns = Invoke-MgGraphRequest -Method GET `
            -Uri "https://graph.microsoft.com/v1.0/auditLogs/signIns?`$filter=riskLevelAggregated eq 'high' and createdDateTime ge $thirtyDaysAgo&`$top=100&`$select=userPrincipalName,riskLevelAggregated,riskState,createdDateTime,ipAddress" `
            -ErrorAction Stop

        $riskyCount = if ($riskySignIns.value) { $riskySignIns.value.Count } else { 0 }
        $uniqueRiskyUsers = if ($riskySignIns.value) {
            ($riskySignIns.value | Select-Object -ExpandProperty userPrincipalName | Sort-Object -Unique).Count
        } else { 0 }

        $results.Add((New-AssessmentResult `
            -CheckName 'USR-004: High-Risk Sign-In Attempts (Last 30 Days)' `
            -Status    (if ($riskyCount -eq 0) { 'Pass' } else { 'Warning' }) `
            -Detail    "High-risk sign-in events in last 30 days: $riskyCount across $uniqueRiskyUsers unique user(s). Review Identity Protection for details." `
        ...
        $results.Add((New-AssessmentResult `
            ...
            -Status    'Info' `
            -Detail    "Check skipped: insufficient permissions or API error. Required: AuditLog.Read.All, IdentityRiskyUser.Read.All. Error: $_" `
            -Recommendation 'Grant AuditLog.Read.All to the service principal. Entra ID P2 license required for risk data.' `
            -Reference 'https://learn.microsoft.com/entra/id-protection/howto-identity-protection-investigate-risk' `
            -Category  'Identity' `
            -Severity  'Info'))
    }
